...
const Navbar = ({ variant = "dark" }) => {
  ...
  return (
    <nav
      className={`fixed top-0 left-0 w-full z-50 transition-all duration-300 ${bgStyle}`}
    >
      <div className="max-w-7xl mx-auto px-4 py-4 flex items-center justify-between">
        ...
        <ul className={`hidden md:flex items-center gap-8 ${textColor}`}>
          <li>
            <Link
              to="/properties"
              className="flex items-center gap-2 hover:text-[#c6a46c]"
            >
              <Building2 size={18} />
              Properties
            </Link>
        ...
        <div className="flex items-center gap-3">
          ...
          {user ? (
            ...
            <Link
              to="/login"
              className="px-4 py-2 rounded-full border border-[#c6a46c] text-white hover:bg-[#c6a46c] hover:text-black transition-all duration-300"
            >
              Login
            </Link>
          )}
